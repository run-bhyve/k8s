...
then
 echo "--anonymous-auth=false --authorization-mode=RBAC --basic-auth-file=$CERTIFICATE/certs/basic_auth.csv --admission-control=NamespaceLifecycle,LimitRanger,ServiceAccount,PersistentVolumeLabel,DefaultStorageClass,DefaultTolerationSeconds,ResourceQuota --token-auth-file=$CERTIFICATE/certs/known_tokens.csv --service-account-key-file=$CERTIFICATE/certs/server.key"
fi` \
`if [ $ENABLE_OIDC == 'true' ]
then
