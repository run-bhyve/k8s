...
`sed -i "s/option domain-name-servers ns1.example.org, ns2.example.org;/option domain-name-servers 10.0.0.1, $CLOUD_HOST_IP;/" /etc/dhcp/temp-local-zones`
`sed -i '/^max-lease-time 7200;/ a\subnet 10.0.0.0 netmask 255.255.255.0 {' /etc/dhcp/temp-local-zones`
`sed -i '/subnet 10.0.0.0 netmask 255.255.255.0 {/ a\option routers 10.0.0.1;'  /etc/dhcp/temp-local-zones`
`sed -i '/option routers 10.0.0.1;/ a\option subnet-mask 255.255.255.0;' /etc/dhcp/temp-local-zones`
`sed -i '/option subnet-mask 255.255.255.0;/ a\option time-offset -18000;' /etc/dhcp/temp-local-zones`
`sed -i '/option time-offset -18000;/ a\range 10.0.0.1 10.0.0.254;' /etc/dhcp/temp-local-zones`
`sed -i '/range 10.0.0.1 10.0.0.254;/ a\}' /etc/dhcp/temp-local-zones`
...
if [ -z "$STATUS" ]
then
`sed -i '/broadcast 10.0.0.254/ a\pre-up iptables-restore < /etc/iptables.rules' /etc/network/interfaces`
fi
...
echo "/root 10.0.0.0/24(rw,async,no_root_squash)" > /etc/exports
echo "/home 10.0.0.0/24(rw,async,no_root_squash)" >> /etc/exports
echo "/export 10.0.0.0/24(rw,async,no_root_squash)" >> /etc/exports
...
`sed -i '/^restrict ::1$/a\ restrict 10.0.0.0 mask 255.255.255.0 nomodify notrap' /etc/ntp.conf`
